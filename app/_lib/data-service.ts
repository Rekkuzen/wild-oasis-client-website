...
export const getCabins = async function ({ filter }: { filter: string }) {
  let query = supabase
    .from("cabins")
  ...
  if (filter === "small")
    query = query.gte("maxCapacity", 1).lte("maxCapacity", 3);
  if (filter === "medium")
    query = query.gte("maxCapacity", 4).lte("maxCapacity", 6);
...
export async function getGuest(email: string) {
  ...
  if (error) {
    console.error(error);
    throw new Error("Guests can't be get..");
  }
